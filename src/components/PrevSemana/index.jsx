      ...
      <div className="hidden md:flex md:flex-row md:flex-1 md:justify-evenly lg:justify-between gap-x-4 flex-wrap gap-4">
        {data.map((item, index) => {
                    return(
                        ...
                        ">
                            <p className="capitalize font-semibold text-lg 2xl:text-3xl">{moment(item.date).format('ddd')}.</p>
                            <img className="h-16 w-16 2xl:h-28 2xl:w-28" 
                            src={`/icons/${item.icon}.png`}
                            alt={`${item.icon}`}
                            />
                            <div className="flex gap-1">
                                <p className="font-medium 2xl:text-3xl">{Math.trunc(item.maxTemp)}°</p>
                                <p className="text-gray-400 font-medium 2xl:text-3xl">/ {Math.trunc(item.minTemp)}°</p>
                            </div>
                        </div>
                    )
        })}
